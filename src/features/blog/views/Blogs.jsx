...
export default function Blogs({ selectedArticleId, selectArticle, navigate }) {
    ...
    async function fetchData() {
      ...
      try {
        if (selectedArticleId) {
          const res = await fetch(`/api/blogs/${selectedArticleId}`);
          if (res.ok) {
            const data = await res.json();
            setActiveblog(data.article);
          }
        } else {
  ...
  // Handle Dynamic Meta & Schema Injection on Active blog Change
  useEffect(() => {
    if (activeblog) {
      document.title = activeblog.meta_title || `${activeblog.title} — privatesector.ch`;
      
      const metaDesc = document.querySelector('meta[name="description"]');
      if (metaDesc) {
        metaDesc.setAttribute('content', activeblog.meta_description || activeblog.subtitle || '');
      }
      ...
      if (activeblog.schema_markup) {
        const script = document.createElement('script');
        script.id = 'seo-jsonld-schema';
        script.type = 'application/ld+json';
        script.innerHTML = activeblog.schema_markup;
        document.head.appendChild(script);
      }
    } else {
      document.title = 'Blogs & Market Analysis — privatesector.ch';
    }
  }, [activeblog]);
  ...
  const uniqueCategories = [...new Set(blogs.map(art => art.category))].map(cat => ({
    name: cat,
    count: blogs.filter(art => art.category === cat).length
  }));

  const uniqueTags = [...new Set(blogs.flatMap(art => {
    try {
  ...
  // Filtering Logic
  const filteredblogs = blogs.filter(art => {
  ...
  // --- 1. SINGLE blog VIEW ---
  if (selectedArticleId && activeblog) {
    // Parse tag array
    let tagsList = [];
    try {
      tagsList = JSON.parse(activeblog.tags || '[]');
    } catch {
      tagsList = [];
    }
      ...
      <div style={{ backgroundColor: 'var(--bg-ivory)', minHeight: 'calc(100vh - 120px)', padding: '32px 0 64px' }}>
        <div className="container">
          <SeoHead
            ...
            description={activeblog.meta_description || activeblog.subtitle}
            image={activeblog.image_url}
            type="blog"
            schemaMarkup={activeblog.schema_markup}
            entityData={activeblog}
          />
          ...
          <Breadcrumbs 
            paths={[
              ...
              { name: activeblog.category },
              { name: activeblog.title }
            ]} 
            navigate={(path) => {
              if (path === '/blogs') selectArticle(null);
              else navigate(path);
            }} 
          />
          ...
          <div style={{
            ...
            {/* Left Column: blog Body */}
            <blog style={{ maxWidth: '720px', width: '100%' }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '16px', alignItems: 'center', padding: '16px 0', borderTop: '0.5px solid var(--light-border)', borderBottom: '0.5px solid var(--light-border)', marginBottom: '32px', fontSize: '13px', color: '#6B7280' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  ...
                  <strong style={{ color: 'var(--text-ink)' }}>{activeblog.author_name}</strong>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                  <Calendar size={14} />
                  <span>{activeblog.date_published}</span>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                  <Clock size={14} />
                  <span>{activeblog.read_time_mins} min read</span>
                </div>
              </div>
                ...
                <div style={{ width: '100%', maxHeight: '420px', overflow: 'hidden', borderRadius: '6px', marginBottom: '32px' }}>
                  <img 
                    ...
                    onError={(e) => {
                      ...
                      e.currentTarget.src = 'https://images.unsplash.com/photo-1554224155-8d04cb21cd6c?auto=format&fit=crop&q=80&w=1200';
                    }}
              ...
              <div className="editorial-content-body" style={{ fontSize: '16px', lineHeight: 1.8, color: 'var(--text-ink)' }}>
                {renderArticleMarkdown(activeblog.content_body)}
              </div>
            ...
            <aside style={{ display: 'flex', flexDirection: 'column', gap: '32px', position: 'sticky', top: '120px' }}>
              <TableOfContents contentHtml={activeblog.content_body} />
              <AdSlot position="D" />
            </aside>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            {filteredblogs.length === 0 ? (
              <div style={{ padding: '48px', backgroundColor: '#FFF', textAlign: 'center', borderRadius: '6px', border: '1px solid #E5E7EB' }}>
                <span style={{ fontSize: '14px', color: '#6B7280' }}>No blogs match your selection.</span>
              </div>
            ) : (
                ...
                >
                  {art.image_url && (
                    <img 
                      ...
                      onError={(e) => {
                        e.currentTarget.onerror = null;
                        e.currentTarget.src = 'https://images.unsplash.com/photo-1554224155-8d04cb21cd6c?auto=format&fit=crop&q=80&w=600';
                      }}
