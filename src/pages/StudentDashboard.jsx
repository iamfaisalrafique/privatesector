...
export default function StudentDashboard({ studentId, navigate }) {
  ...
  if (loading) {
    return (
      <div className="container" style={{ padding: '64px 0', textAlign: 'center' }}>
        ...
        <p style={{ fontFamily: '"Playfair Display", serif', fontSize: '18px', color: 'var(--text-charcoal)' }}>Loading dashboard...</p>
      </div>
    );
  }
  ...
  if (!student) {
    return (
      <div className="container" style={{ padding: '64px 0', textAlign: 'center' }}>
        <h2 style={{ fontFamily: '"Playfair Display", serif', fontSize: '24px', color: 'var(--text-ink)', marginBottom: '16px' }}>Student Profile Not Found</h2>
        <button className="btn btn-gold-fill" onClick={() => navigate('/karriere')}>Go to Careers Center</button>
      </div>
    );
  }
    ...
    <div style={{ backgroundColor: 'var(--bg-ivory)', minHeight: 'calc(100vh - 120px)', padding: '40px 0 64px' }}>
      <div className="container">
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#0A0A0A', color: '#FFFDF7', padding: '32px', borderRadius: '6px', borderBottom: '1.5px solid rgba(191,155,48,0.3)', marginBottom: '32px' }}>
          <div>
            <span style={{ color: 'var(--primary-red)', fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.1em', fontWeight: 700 }}>Student Talent Hub</span>
            <h1 style={{ fontSize: '28px', fontFamily: '"Playfair Display", serif', margin: '4px 0 0' }}>Welcome, {student.name}!</h1>
          </div>
          <button className="btn btn-gold-fill" onClick={() => navigate(`/student/${studentId}`)}>
            <ArrowLeft size={14} style={{ marginRight: '6px' }} /> View Public Profile
          </button>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '250px 1fr', gap: '32px' }} className="student-dashboard-layout">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            ...
            >
              <UserCheck size={16} /> Edit Profile (LinkedIn Style)
            </button>
            <button 
            ...
            >
              <BookOpen size={16} /> Write Business Article
            </button>
          </div>
          ...
          <div style={{ backgroundColor: '#FFFFFF', border: '0.5px solid var(--light-border)', borderRadius: '6px', padding: '32px' }}>
            ...
            {activeTab === 'overview' && (
              <div>
                <h3 style={{ fontSize: '20px', fontFamily: '"Playfair Display", serif', fontWeight: 700, borderBottom: '0.5px solid var(--light-border)', paddingBottom: '12px', marginBottom: '24px' }}>Dossier Overview</h3>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '20px', marginBottom: '40px' }}>
                  <div style={{ backgroundColor: 'var(--bg-ivory)', border: '0.5px solid var(--light-border)', padding: '24px', borderRadius: '4px', textAlign: 'center' }}>
                    <span style={{ fontSize: '12px', color: 'var(--text-charcoal)', textTransform: 'uppercase', fontWeight: 600 }}>Articles Written</span>
                  ...
                  <div style={{ backgroundColor: 'var(--bg-ivory)', border: '0.5px solid var(--light-border)', padding: '24px', borderRadius: '4px', textAlign: 'center' }}>
                    <span style={{ fontSize: '12px', color: 'var(--text-charcoal)', textTransform: 'uppercase', fontWeight: 600 }}>Podcasts & Briefings</span>
                ...
                <h4 style={{ fontSize: '16px', fontFamily: '"Playfair Display", serif', fontWeight: 700, marginBottom: '16px' }}>My Published Pieces</h4>
                {(!student.articles || student.articles.length === 0) ? (
                  <p style={{ fontStyle: 'italic', color: 'var(--text-charcoal)', fontSize: '13px' }}>You haven't published any articles yet. Navigate to 'Write Business Article' to get started!</p>
                ) : (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    {student.articles.map(art => (
                      <div key={art.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px', border: '0.5px solid var(--light-border)', borderRadius: '4px' }}>
                        <div>
                          <span style={{ fontSize: '10px', textTransform: 'uppercase', color: 'var(--primary-red)', fontWeight: 600 }}>{art.category}</span>
                          <strong style={{ display: 'block', fontSize: '14px', color: 'var(--text-ink)', marginTop: '2px' }}>{art.title}</strong>
                        </div>
                        <button className="btn btn-gold-fill" style={{ fontSize: '11px', padding: '6px 12px', minHeight: '32px' }} onClick={() => navigate(`/news/${art.id}`)}>View Article</button>
                      </div>
                    ))}
                  </div>
                )}
            ...
            {activeTab === 'edit-profile' && (
              <form onSubmit={handleProfileSave}>
                <h3 style={{ fontSize: '20px', fontFamily: '"Playfair Display", serif', fontWeight: 700, borderBottom: '0.5px solid var(--light-border)', paddingBottom: '12px', marginBottom: '24px' }}>Edit LinkedIn-Style Profile</h3>
                
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginBottom: '24px' }} className="edit-form-grid">
                  <div>
                    <label style={{ fontSize: '12px', fontWeight: 600, display: 'block', marginBottom: '6px' }}>University</label>
                    <input type="text" value={university} onChange={e => setUniversity(e.target.value)} className="input-field" required />
                  </div>
                  <div>
                    <label style={{ fontSize: '12px', fontWeight: 600, display: 'block', marginBottom: '6px' }}>Field of Study</label>
                    <input type="text" value={studyField} onChange={e => setStudyField(e.target.value)} className="input-field" required />
                  </div>
                  <div>
                    <label style={{ fontSize: '12px', fontWeight: 600, display: 'block', marginBottom: '6px' }}>Graduation Year</label>
                    <input type="number" value={gradYear} onChange={e => setGradYear(e.target.value)} className="input-field" required />
                  </div>
                  <div>
                    <label style={{ fontSize: '12px', fontWeight: 600, display: 'block', marginBottom: '6px' }}>Portfolio URL</label>
                    <input type="url" value={portfolioUrl} onChange={e => setPortfolioUrl(e.target.value)} className="input-field" />
                  </div>
                  <div>
                    <label style={{ fontSize: '12px', fontWeight: 600, display: 'block', marginBottom: '6px' }}>Avatar Image URL</label>
                  ...
                  <div>
                    <label style={{ fontSize: '12px', fontWeight: 600, display: 'block', marginBottom: '6px' }}>Birth Date</label>
                    <input type="date" value={birthDate} onChange={e => setBirthDate(e.target.value)} className="input-field" />
                  </div>
                </div>

                <div style={{ marginBottom: '24px' }}>
                  <label style={{ fontSize: '12px', fontWeight: 600, display: 'block', marginBottom: '6px' }}>Bio / Summary</label>
                ...
                <div style={{ marginBottom: '32px', borderTop: '0.5px solid var(--light-border)', paddingTop: '20px' }}>
                  <h4 style={{ fontSize: '15px', fontFamily: '"Playfair Display", serif', fontWeight: 700, marginBottom: '12px' }}>Skills & Endorsements</h4>
                  ...
                  <div style={{ display: 'flex', gap: '8px' }}>
                    <input type="text" value={newSkill} onChange={e => setNewSkill(e.target.value)} placeholder="Add a skill..." className="input-field" style={{ maxWidth: '240px' }} />
                    <button type="button" onClick={handleAddSkill} className="btn btn-gold-fill" style={{ fontSize: '12px', minHeight: '44px' }}>Add</button>
                  </div>
                </div>

                {/* Experience Sector */}
                <div style={{ marginBottom: '32px', borderTop: '0.5px solid var(--light-border)', paddingTop: '20px' }}>
                  <h4 style={{ fontSize: '15px', fontFamily: '"Playfair Display", serif', fontWeight: 700, marginBottom: '16px' }}>Experience & Projects</h4>
                  
                  {experience.length > 0 && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', marginBottom: '24px' }}>
                      {experience.map((exp, idx) => (
                        <div key={idx} style={{ border: '0.5px solid var(--light-border)', borderRadius: '4px', padding: '16px', position: 'relative' }}>
                          <button type="button" onClick={() => handleRemoveExperience(idx)} style={{ position: 'absolute', top: '16px', right: '16px', border: 'none', background: 'none', color: 'var(--primary-red)', cursor: 'pointer', fontWeight: 700 }}>Remove</button>
                          <strong style={{ fontSize: '14px', display: 'block', color: 'var(--text-ink)' }}>{exp.role}</strong>
                          <span style={{ fontSize: '12px', color: 'var(--primary-red)', fontWeight: 600, display: 'block', marginTop: '2px' }}>{exp.company} · {exp.duration}</span>
                          {exp.description && <p style={{ fontSize: '12px', color: 'var(--text-charcoal)', margin: '8px 0 0', lineHeight: 1.5 }}>{exp.description}</p>}
                        </div>
                      ))}
                    </div>
                  )}
                  ...
                  <div style={{ border: '0.5px solid var(--light-border)', borderRadius: '4px', padding: '20px', backgroundColor: 'var(--bg-ivory)' }}>
                    <h5 style={{ fontSize: '13px', margin: '0 0 16px', textTransform: 'uppercase', color: 'var(--text-charcoal)' }}>Add Experience</h5>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px', marginBottom: '16px' }}>
                      <input type="text" value={newRole} onChange={e => setNewRole(e.target.value)} placeholder="Role (e.g. Intern)" className="input-field" />
                      <input type="text" value={newCompany} onChange={e => setNewCompany(e.target.value)} placeholder="Company / University project" className="input-field" />
                      <input type="text" value={newDuration} onChange={e => setNewDuration(e.target.value)} placeholder="Duration (e.g. 3 months)" className="input-field" />
                    </div>
                    <textarea value={newDesc} onChange={e => setNewDesc(e.target.value)} placeholder="Description..." className="input-field" style={{ minHeight: '80px', marginBottom: '16px', resize: 'vertical' }} />
                    <button type="button" onClick={handleAddExperience} className="btn btn-gold-fill" style={{ fontSize: '12px', minHeight: '36px' }}>Add Experience</button>
                  </div>
                </div>
            ...
            {activeTab === 'write-article' && (
              <form onSubmit={handlePublishArticle}>
                <h3 style={{ fontSize: '20px', fontFamily: '"Playfair Display", serif', fontWeight: 700, borderBottom: '0.5px solid var(--light-border)', paddingBottom: '12px', marginBottom: '24px' }}>Compose Business Article</h3>
                
                <div style={{ marginBottom: '20px' }}>
                  <label style={{ fontSize: '12px', fontWeight: 600, display: 'block', marginBottom: '6px' }}>Title</label>
                  <input type="text" value={artTitle} onChange={e => setArtTitle(e.target.value)} placeholder="Swiss watch exports reach historic heights..." className="input-field" required />
                </div>
                
                <div style={{ marginBottom: '20px' }}>
                  <label style={{ fontSize: '12px', fontWeight: 600, display: 'block', marginBottom: '6px' }}>Subtitle</label>
                  <input type="text" value={artSub} onChange={e => setArtSub(e.target.value)} placeholder="Brief summary teaser..." className="input-field" required />
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginBottom: '20px' }} className="edit-form-grid">
                  <div>
                    ...
                    <select value={artCategory} onChange={e => setArtCategory(e.target.value)} className="input-field" style={{ height: '44px' }}>
                      ...
                      <option value="Street Briefing">Street Briefing</option>
                    </select>
                  </div>
                  <div>
                    <label style={{ fontSize: '12px', fontWeight: 600, display: 'block', marginBottom: '6px' }}>Image URL</label>
                    <input type="url" value={artImage} onChange={e => setArtImage(e.target.value)} placeholder="Unsplash image URL..." className="input-field" />
                  </div>
                </div>

                <div style={{ marginBottom: '20px' }}>
                  <label style={{ fontSize: '12px', fontWeight: 600, display: 'block', marginBottom: '6px' }}>Tags (Comma separated)</label>
                  <input type="text" value={artTags} onChange={e => setArtTags(e.target.value)} placeholder="Rolex, Richemont, ESG" className="input-field" />
                </div>

                <div style={{ marginBottom: '20px' }}>
                  <label style={{ fontSize: '12px', fontWeight: 600, display: 'block', marginBottom: '6px' }}>Pull Quote (Featured text highlight)</label>
                  <input type="text" value={artQuote} onChange={e => setArtQuote(e.target.value)} placeholder="Pull quote highlight..." className="input-field" />
                </div>

                <div style={{ marginBottom: '24px' }}>
                  <label style={{ fontSize: '12px', fontWeight: 600, display: 'block', marginBottom: '6px' }}>Body Content</label>
